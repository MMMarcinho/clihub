import { spawn } from "child_process";
import * as path from "path";
import { StepResult, Workflow, WorkflowStep } from "./types";
import { renderTemplate, renderShellCommand, emptyContext, TemplateContext } from "./template";
import { applySelect, parseCaptureFormat } from "./capture";

export interface RunOptions {
  cwd?: string;
  onStepStart?: (id: string, command: string) => void;
  onStepEnd?: (result: StepResult) => void;
}

export interface RunOutcome {
  success: boolean;
  results: StepResult[];
}

interface SpawnResult {
  stdout: string;
  stderr: string;
  status: number | null;
}

function spawnAsync(command: string, cwd: string, env: NodeJS.ProcessEnv): Promise<SpawnResult> {
  return new Promise((resolve, reject) => {
    const child = spawn(command, { shell: true, cwd, env });
    let stdout = "";
    let stderr = "";
    child.stdout!.setEncoding("utf8");
    child.stderr!.setEncoding("utf8");
    child.stdout!.on("data", (chunk: string) => {
      stdout += chunk;
    });
    child.stderr!.on("data", (chunk: string) => {
      stderr += chunk;
    });
    child.on("error", reject);
    child.on("close", (status) => resolve({ stdout, stderr, status }));
  });
}

/**
 * Runs one step's `run`/`capture`/`assign` against the shared template
 * context. Used both for ordinary sequential steps and for the members of a
 * `parallel` group — callers awaiting several of these concurrently must
 * make sure those steps don't reference each other's results, since there's
 * no ordering guarantee between them (an unresolved steps/captures/vars
 * template reference throws, so a same-group dependency fails loudly rather
 * than silently reading a value from the wrong point in time).
 */
async function runSingleStep(
  step: WorkflowStep,
  ctx: TemplateContext,
  baseCwd: string,
  onStepStart?: (id: string, command: string) => void,
  onStepEnd?: (result: StepResult) => void
): Promise<{ result: StepResult; failed: boolean }> {
  let result: StepResult;
  let captureFailed = false;

  if (step.run) {
    // Human-readable command for display/audit (StepResult.command, progress
    // output): values fully resolved and shell-quoted so nothing is hidden.
    const displayCommand = renderTemplate(step.run, ctx, "shell");
    onStepStart?.(step.id, displayCommand);

    // Actual command executed: references become "$VAR" env-var expansions
    // instead of literal text, so previous-step output can never be
    // reinterpreted as shell syntax (see renderShellCommand doc comment).
    const { command: execCommand, env: refEnv } = renderShellCommand(step.run, ctx);

    const stepCwd = step.cwd ? path.resolve(baseCwd, step.cwd) : baseCwd;
    const env = { ...process.env, ...refEnv, ...(step.env ?? {}) };

    const start = Date.now();
    const proc = await spawnAsync(execCommand, stepCwd, env);
    const durationMs = Date.now() - start;

    result = {
      id: step.id,
      command: displayCommand,
      stdout: proc.stdout,
      stderr: proc.stderr,
      exitCode: proc.status,
      durationMs,
    };

    if (step.capture) {
      try {
        const parsed = parseCaptureFormat(step.capture.format, result.stdout);
        result.parsed = parsed;
        ctx.captures[step.capture.as] = applySelect(parsed, step.capture.select);
      } catch (err) {
        result.stderr += (result.stderr ? "\n" : "") + `capture error: ${(err as Error).message}`;
        captureFailed = true;
      }
    }

    ctx.steps[step.id] = {
      stdout: result.stdout,
      stderr: result.stderr,
      exitCode: result.exitCode,
      parsed: result.parsed,
    };
  } else {
    result = { id: step.id, command: "", stdout: "", stderr: "", exitCode: 0, durationMs: 0 };
  }

  if (step.assign) {
    for (const [varName, tmpl] of Object.entries(step.assign)) {
      ctx.vars[varName] = renderTemplate(tmpl, ctx, "plain");
    }
  }

  onStepEnd?.(result);

  const rawFailed = (step.run !== undefined && result.exitCode !== 0) || captureFailed;
  return { result, failed: rawFailed && !step.continueOnError };
}

function pushSkipped(step: WorkflowStep, results: StepResult[]): void {
  if (step.parallel) {
    for (const child of step.parallel) pushSkipped(child, results);
    return;
  }
  results.push({
    id: step.id,
    command: step.run ?? "",
    stdout: "",
    stderr: "",
    exitCode: null,
    durationMs: 0,
    skipped: true,
  });
}

export async function runWorkflow(
  workflow: Workflow,
  inputs: Record<string, string>,
  options: RunOptions = {}
): Promise<RunOutcome> {
  const baseCwd = options.cwd ?? process.cwd();
  const results: StepResult[] = [];
  const ctx: TemplateContext = emptyContext(inputs);
  let success = true;

  for (const step of workflow.steps) {
    if (!success) {
      pushSkipped(step, results);
      continue;
    }

    if (step.parallel) {
      const outcomes = await Promise.all(
        step.parallel.map((child) => runSingleStep(child, ctx, baseCwd, options.onStepStart, options.onStepEnd))
      );
      for (const outcome of outcomes) {
        results.push(outcome.result);
      }
      if (outcomes.some((outcome) => outcome.failed)) {
        success = false;
      }
      continue;
    }

    const { result, failed } = await runSingleStep(step, ctx, baseCwd, options.onStepStart, options.onStepEnd);
    results.push(result);
    if (failed) {
      success = false;
    }
  }

  return { success, results };
}
